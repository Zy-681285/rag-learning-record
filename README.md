...
我目前在三大运营商做驻店专员，负责办理运营商购机优惠，今年3月份分配到新的门店，店员都没有接触过这项业务。平时他们也都很忙，很少能有时间凑在一起，完整接受系统性的学习。
以此为契机，我便动手搭建内部运营商信息查询RAG，主要是一些基础信息的查询，以及不同套餐的对比，针对每个套餐的不同维度都做了完整的数据。
花了一个月左右，用dify搭建的RAG集成到钉钉使用，使用的是langbot 来链接，检索效率不稳定，有时候要将近1分钟。
现在是采用了Langchain的框架，这次是集成到Web端，一开始使用无影云电脑，开放端口异常复杂，最终是选用了阿里云服务器，检索效率控制在5秒以内。
我没有技术背景，做的是销售性质的工作。目前仍在持续调优，同时系统性学习LangChain底层原理，计划以此为方向完成职业转型。
